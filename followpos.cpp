...
int main(){
  ...
  //cout<<"Postfix: "<<postfix<<endl<<endl;  //prints postfix expression
...
bool is_valid(string input){
  ...
  for(int i = 1; i< input.size(); i++){
    ...
    if(input[i-1] == '(' || input[i-1] == ')'){
      if(input[i-1] == '(') bracket_balance.push('(');
      else if( !bracket_balance.empty() && input[i-1] == ')') bracket_balance.pop();
      else if(input[i-1] == ')' && bracket_balance.empty())  return false;
    }
  }
  if( !bracket_balance.empty() ) return false;
  return true;
}
